  ...
  private:
    ...
    class QuadtreeNode
    {
      public:
        ...
        QuadtreeNode(int xc, int yc);
    };

    QuadtreeNode* root; /**< pointer to root of quadtree */
    int res; // resolution of the underlying bitmap

    RGBAPixel avgChildren(QuadtreeNode* nw, QuadtreeNode* ne, QuadtreeNode* sw, QuadtreeNode* se);

    RGBAPixel getPixelHelper(int x, int y, int resolution, QuadtreeNode* curr, int cx, int cy) const;


    //helper for copy constructor
    void buildRow(QuadtreeNode* q, int setresolution, PNG const& source);

    void buildLvls(int setresolution, QuadtreeNode* n);

    void buildRowFromTree(QuadtreeNode* q, int setresolution, QuadtreeNode* oq);

    void rotateChildren(QuadtreeNode* q);

    void pruneHelper(int tolerance, QuadtreeNode* &curr);

    bool colorDiff(QuadtreeNode* curr, QuadtreeNode* childof, int tolerance) const;

    int pruneSizeHelper(int tolerance, QuadtreeNode* curr, QuadtreeNode* orig) const;

    bool pruneSizeHelper2(int tolerance, QuadtreeNode* curr, QuadtreeNode* orig) const;

    int idealPruneHelper(int numLeaves, int min, int max) const;

    void destructorHelper(QuadtreeNode* &curr);
